...
func setup() -> void:
	...
	if !MemoryManager.memory.dirr_at_2 && !MemoryManager.memory.dirr_follow_player:
		await get_tree().process_frame
		await get_tree().process_frame
	...
	if MemoryManager.memory.dirr_follow_player:
		var dirr : Dirr = get_tree().get_first_node_in_group("Dirr")
		if dirr != null:
			dirr.follow_player()
		else:
			var dirr_load = preload(dirr_prefab)
			dirr = dirr_load.instantiate()
			get_tree().current_scene.add_child(dirr)
			dirr.follow_player()
		dirr.global_position = lowerSpawn.global_position + Vector2(0, -50)
	elif !MemoryManager.memory.dirr_spawned_03 && (MemoryManager.memory.window_race || MemoryManager.memory.dirr_go_home):
		var old_dirr : Dirr = get_tree().get_first_node_in_group("Dirr")
		if old_dirr != null:
			old_dirr.queue_free()
		...
		dirr_instance.move_through_gate_2()
		dirr_instance.state_move_through_gate_2 = true
		dirr_instance.static_collider.disabled = true
		
		MemoryManager.memory.dirr_spawned_03 = true
